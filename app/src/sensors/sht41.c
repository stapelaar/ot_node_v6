...
/* ============================================================
 * PUBLIC API (called by app_core sample thread)
 * ============================================================ */
...
void sht41_sample_and_publish(const char *root)
{
    ...
    LOG_INF("SHT41 OK: T=%d.%02d C, RH=%d.%02d %%",
            t_centi / 100, abs(t_centi % 100),
            rh_centi / 100, abs(rh_centi % 100));

    /* Publish both fields back-to-back — the OT mutex is acquired and
       released inside each transport_publish() call. No sleep between
       them: sleeping here would starve the OT task between two lock
       acquisitions and cause the second lock to block forever. */
